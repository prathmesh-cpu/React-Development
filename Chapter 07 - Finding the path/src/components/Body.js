...
const filterData = (searchText, listOfRestaurants) => {
    const filteredList = listOfRestaurants.filter((restaurant) => {
        return (restaurant?.info?.name.toLowerCase().includes(searchText.toLowerCase())) || (restaurant?.info?.cuisines.join(" ").toLowerCase().includes(searchText.toLowerCase()));
    })
    return filteredList;
}
                    ...
                    <div className='restaurantContainer'>
                        {
                            filteredRestaurant.map((restaurantItem) => (
                                <Link to={"restaurants/" + restaurantItem.info.id} key={restaurantItem.info.id}>
